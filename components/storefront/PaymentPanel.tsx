    ...
    return (
      <section id="active-checkout" className="ml-9 max-w-[430px] bg-success-bg border border-success-border rounded-xl p-4 flex flex-col gap-2">
        ...
        <span className="text-[13px] text-success-ink leading-relaxed">
          Order <span className="font-mono">{order.order_id}</span> is confirmed. The Paytm
          simulator verified the payment for this exact order and amount.
        </span>
        <button
    ...
    <section id="active-checkout" className="ml-9 max-w-[430px] bg-white border border-[#cfd9d5] rounded-xl overflow-hidden shadow-sm">
      <div className="px-4 pt-4 pb-3 flex flex-col gap-2.5">
        <div className="flex justify-between items-baseline">
          ...
          <span className="font-mono text-[10px] text-ink-faint">PAYTM · SIMULATED</span>
        </div>
      ...
      <div className="px-4 pb-4 flex flex-col gap-2">
        ...
        {declined ? (
          ...
          >
            Pay {formatMinor(order.total_minor)} with Paytm
          </a>
        ) : (
          <button
            onClick={() => run("retry", () => retryPayment(order.order_id))}
        ...
        <span className="text-[11px] text-ink-faint text-center leading-relaxed">
          ...
          {declined
            ...
            : awaiting
              ? "Waiting on Paytm to confirm this payment. We're checking automatically — no need to refresh."
              : "Simulated Paytm checkout — no real money moves. This order is marked paid only once the gateway confirms this exact order and amount, and this page updates on its own once it does."}
        </span>
      </div>
      <OrderProvenance order={order} />
    </section>
