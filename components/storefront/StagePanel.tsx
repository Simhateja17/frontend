  ...
  return (
    <section className="ml-9 max-w-[430px] bg-white border border-[#cfd9d5] rounded-xl overflow-hidden shadow-sm">
      ...
      <div className="px-4 pb-4 flex flex-col gap-2">
        ...
        >
          {confirmingCheckout ? "Opening Paytm…" : `Pay ${formatMinor(stage.total_minor)} with Paytm`}
        </button>
        <button
          onClick={cancelStage}
          disabled={confirmingCheckout}
          className="w-full bg-white text-ink border border-border rounded-lg py-2 text-[12.5px] hover:bg-bg transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
        >
          Keep shopping
        </button>
        ...
        <span className="text-[11px] text-ink-faint text-center leading-relaxed">
          You’ll go straight to Paytm. Your order is created and your items are held
          while you pay.
        </span>
      </div>
    </section>
