...
function CatalogProductImage({ title, category, priority = false, detailed = false }: {
  title: string;
  category: string | null;
  priority?: boolean;
  detailed?: boolean;
}) {
  const imageUrl = getVariantImageUrl({ title, category }, detailed ? 800 : 480);
  ...
  if (imageFailed) {
    ...
    return detailed ? (
      <div className="text-center p-8">
        <span aria-hidden="true" className="text-7xl text-accent">{icon}</span>
        <p className="text-sm mt-4 text-ink-muted">{category}</p>
        <p className="text-xs text-ink-faint">Product image unavailable</p>
      </div>
  ...
  return (
    <Image
      src={imageUrl}
      alt={title}
      width={detailed ? 800 : 480}
      height={detailed ? 800 : 480}
  ...
  return <section className="max-w-6xl mx-auto p-4 sm:p-6" aria-label="Browse products">
    {selected && variant ? <>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl bg-surface-muted min-h-48 grid place-items-center border border-border-soft overflow-hidden"><CatalogProductImage title={selected.title} category={selected.category} priority detailed /></div>
      ...
      {loading ? <p role="status">Loading the collection…</p> : error ? <div role="alert"><p>{error}</p><button className="text-accent mt-2" onClick={() => { setLoading(true); setReload(r => r + 1); }}>Try again</button></div> : <>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">{visible.map((product, index) => <button key={product.product_id} className="text-left rounded-xl border border-border-soft bg-white overflow-hidden hover:border-accent transition-colors" onClick={() => select((product.variants.find(v => v.in_stock && v.price_minor === product.from_price_minor) ?? product.variants[0]).variant_id)}><div className="h-28 sm:h-36 bg-surface-muted grid place-items-center text-5xl text-accent overflow-hidden"><CatalogProductImage title={product.title} category={product.category} priority={index < 10} /></div><div className="p-3"><p className="text-[11px] text-ink-muted">{product.brand} · {product.category}</p><h2 className="font-medium text-sm mt-1">{product.title}</h2><p className="font-semibold mt-2">From {formatMinor(product.from_price_minor)}</p><p className="text-xs mt-1 text-ink-muted">{product.in_stock ? `${product.variants.length} options` : "Out of stock"}</p></div></button>)}</div>
      </>}
    </>}
  </section>;
}
